...
export const googleCallbackController = async (req, res) => {
    try {
        console.log('------Callback req.user: -------', req.user); // Thêm dòng này

        if (!req.user || !req.user.id) {
            console.error('❌ No user data or missing id:', req.user);
            return res.status(400).json({ err: 1, msg: 'No user data from Google' });
        }
...
export const facebookCallbackController = async (req, res) => {
    try {
        ...
        if (!req.user || !req.user.id) {
            console.error('❌ No user data or missing id:', req.user);
            return res.status(400).json({ err: 1, msg: 'No user data from Facebook' });
        }
